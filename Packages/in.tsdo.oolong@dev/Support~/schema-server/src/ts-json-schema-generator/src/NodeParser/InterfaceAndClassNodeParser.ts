...
import { isUnitySerializable } from '../Utils/unity';
...
export class InterfaceAndClassNodeParser implements SubNodeParser {
  ...
  protected getProperties(
    ...
    const properties = (node.members as ts.NodeArray<ts.TypeElement | ts.ClassElement>)
      ...
      .filter(member => isUnitySerializable(member) && !isStatic(member) && !isNodeHidden(member))
  ...
  protected getAdditionalProperties(
    ...
    const indexSignature = (node.members as ts.NodeArray<ts.NamedDeclaration>).find(
    ...
    return (
      this.childNodeParser.createType(indexSignature.type!, context) ?? this.additionalProperties
    );
  }
